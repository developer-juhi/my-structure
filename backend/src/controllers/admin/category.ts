...
const destroy = (async (req: Request, res: Response) => {
    ...
    session.startTransaction();
    try {
        await Category.deleteMany({ _id: req.query.id, })
...
const getData = (async (id: number) => {
    ...
    return categoryData.length > 0 ? categoryData[0] : {};
});
...
const changeStatus = (async (req: Request, res: Response) => {
    ...
    try {
        ...
        const categoryData: any = await Category.findOne({ _id: id });
        categoryData.is_active = status;
        await categoryData.save();
        const message: string = `Category status ${(status === "true") ? 'Approved' : 'Rejected'} successfully`
        const responseData: any = {
            'message': message,
            'data': true,
        };
...
const store = (async (req: Request, res: Response) => {
    ...
    try {
        ...
        let message: any
        if (id) {
            categoryData = await Category.findOne({ _id: id });
            message = 'Category updated successfully';
        } else {
            categoryData = await new Category();
            message = 'Category added successfully';
        }
        categoryData.name = name;
        if (parent_id) {
            categoryData.parent_id = new mongoose.Types.ObjectId(parent_id);
        }
        await categoryData.save();
        await session.commitTransaction();
