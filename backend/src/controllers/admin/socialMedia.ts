...
const destroy = (async (req: Request, res: Response) => {
    ...
    session.startTransaction();
    try {
        await SocialMedia.deleteMany({ _id: req.query.id, })
...
const changeStatus = (async (req: Request, res: Response) => {
    ...
    try {
        ...
        const socialMediaData: any = await SocialMedia.findOne({ _id: id });
        // socialMediaData.is_active = status;
        await socialMediaData.save();
        const message: string = `SocialMedia status ${(status === "true") ? 'Approved' : 'Rejected'} successfully`
        const responseData: any = {
            'message': message,
            'data': true,
        };
...
const store = (async (req: Request, res: Response) => {
    ...
    try {
        ...
        let message: any
        if (id) {
            socialMediaData = await SocialMedia.findOne({ _id: id });
            message = 'Social Media updated successfully';
        } else {
            socialMediaData = await new SocialMedia();
