...
import { faPencilAlt, faTrashAlt, faEye, faToggleOff, faToggleOn, faComment, faList } from '@fortawesome/free-solid-svg-icons';
...
const Index = () => {
    ...
    const editButtonClick = (id) => {
        navigate('/customer/form', { state: { id: id } });
    };
    ...
    const serviceRequestButtonClick = (row) => {
        navigate('/service-request', { state: { user_id: row._id, user_name: row.first_name + " " + row.last_name } });
    };
    const assetButtonClick = (row) => {
        navigate('/asset', { state: { user_id: row._id, user_name: row.first_name + " " + row.last_name } });
    };
    ...
    return (
        <Fragment>
            ...
            <Modal title="Customer Details" centered footer={''} open={visible} onCancel={handleViewModelCancel}>
                {viewModalText}
            </Modal>
        </Fragment >
    );
